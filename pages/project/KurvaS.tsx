...
const EditableCell = ({ value, onChange, type = 'text', className = '' }: { value: string | number | null, onChange: (value: any) => void, type?: string, className?: string }) => {
    const commonProps = {
        className: `w-full bg-transparent p-1 focus:bg-white dark:focus:bg-gray-700 rounded-md outline-none focus:ring-1 focus:ring-primary text-inherit ${className}`
    };
...
const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-card/80 dark:bg-popover/80 backdrop-blur-sm p-2 border border-border rounded-lg shadow-lg text-xs">
                <p className="font-bold mb-1 text-card-foreground">{`Minggu ke-${payload[0].payload.week}`}</p>
                {payload.map((pld: any) => (
...
const RincianLaporanView = ({ tasks, projectName, onBack, timeline, manualWeeklyProgress }: RincianLaporanViewProps) => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans p-0 sm:p-0 lg:p-0 print:p-2 print:bg-white print:dark:bg-white print:text-black">
            <style>
                {`
                    @media print {
                        body * {
            ...
            <div className="bg-card p-6 rounded-xl shadow-lg print:shadow-none print:rounded-none printable-area">
                <header className="flex flex-col md:flex-row justify-between items-center mb-6 border-b dark:border-gray-700 pb-4 print:hidden">
                    <div>
                        <h1 className="font-bold text-2xl text-card-foreground">Rincian Laporan Proyek</h1>
                    ...
                    <div className="flex items-center gap-4 mt-4 md:mt-0">
                         <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                className="bg-card border border-input rounded-md px-3 py-1.5 text-sm focus:ring-primary focus:border-primary"
                            >
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={handlePrint} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-foreground bg-muted rounded-md hover:bg-muted/80 transition">
                                <Printer size={16} /> Cetak
                            </button>
                            <button onClick={onBack} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-primary-foreground bg-primary rounded-md hover:bg-primary/90 transition shadow">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse text-xs whitespace-nowrap">
                        <thead>
                            <tr className="bg-primary/10 text-primary uppercase print:bg-gray-200 print:text-black">
                        ...
                        <tbody>
                            {tasks.map(task => {
                                ...
                                return (
                                    <tr key={task.id} className={`${task.isCategory ? 'bg-muted/50 font-bold print:bg-gray-100' : 'hover:bg-muted/30'}`}>
                                        <td className="border border-border p-2 text-center">{taskNo}</td>
                                        <td className="border border-border p-2">{task.name}</td>
                                        <td className="border border-border p-2 text-center">{!task.isCategory && bobotMingguan > 0 ? formatDecimal(bobotMingguan) : ''}</td>
                                        <td className="border border-border p-0 print:p-1">
                                            {!task.isCategory ? <input type="text" value={taskProgress.mingguLalu} onChange={e => handleProgressChange(task.id, 'mingguLalu', e.target.value)} className="w-full h-full bg-transparent p-2 text-center outline-none focus:ring-1 focus:ring-primary print:ring-0" placeholder="0,00"/> : <div className="p-2"></div>}
                                        </td>
                                        <td className="border border-border p-0 print:p-1">
                                            {!task.isCategory ? <input type="text" value={taskProgress.mingguIni} onChange={e => handleProgressChange(task.id, 'mingguIni', e.target.value)} className="w-full h-full bg-transparent p-2 text-center outline-none focus:ring-1 focus:ring-primary print:ring-0" placeholder="0,00"/> : <div className="p-2"></div>}
                                        </td>
                                        <td className="border border-border p-2 text-center bg-muted/50">
                                            {!task.isCategory ? formatDecimal(pSdIni) : ''}
                                        </td>
                                        <td className="border border-border p-0 print:p-1">
                                            <input type="text" value={keterangan[task.id] || ''} onChange={e => handleKeteranganChange(task.id, e.target.value)} className="w-full h-full bg-transparent p-2 outline-none focus:ring-1 focus:ring-primary print:ring-0"/>
                                        </td>
                                    </tr>
                                );
                            })}
                        ...
                        <tfoot>
                            <tr className="bg-primary/20 font-bold print:bg-gray-200">
                                <td colSpan={2} className="border border-border p-2 text-center">JUMLAH</td>
                                <td className="border border-border p-2 text-center">{formatDecimal(totals.bobot)}</td>
                                <td className="border border-border p-2 text-center">{formatDecimal(totals.mingguLalu)}</td>
                                <td className="border border-border p-2 text-center">{formatDecimal(totals.mingguIni)}</td>
                                <td className="border border-border p-2 text-center">{formatDecimal(totals.sdMingguIni)}</td>
...
const KurvaS = () => {
    ...
    return (
        <div className="bg-card p-4 rounded-xl shadow-lg font-sans">
            <header className="mb-4 space-y-4">
                 <h1 className="font-bold text-xl text-card-foreground">Jadwal Proyek (Kurva S)</h1>
                 <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg border">
                    <div>
                        <label className="text-xs font-semibold text-muted-foreground block mb-1">ID Project</label>
                        <input type="text" value={idProject} onChange={e => setIdProject(e.target.value)} className="w-full text-sm p-2 border rounded-md bg-background" />
                    </div>
                    <div>
                        <label className="text-xs font-semibold text-muted-foreground block mb-1">Nama Proyek</label>
                        <input type="text" value={projectName} onChange={e => setProjectName(e.target.value)} className="w-full text-sm p-2 border rounded-md bg-background" />
                    </div>
                    <div>
                        <label className="text-xs font-semibold text-muted-foreground block mb-1">Tanggal Rencana Mulai</label>
                        <input type="date" value={projectStartDate} onChange={e => setProjectStartDate(e.target.value)} className="w-full text-sm p-2 border rounded-md bg-background" />
                    </div>
                    <div>
                        <label className="text-xs font-semibold text-muted-foreground block mb-1">Durasi Proyek (Hari Kalender)</label>
                        <input type="number" value={projectDuration} onChange={e => setProjectDuration(Number(e.target.value))} className="w-full text-sm p-2 border rounded-md bg-background" />
                    </div>
                 </div>
                 <div className="flex items-center gap-2">
                     ...
                     <button onClick={() => handleAddNewItem(false)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-primary-foreground bg-primary rounded-md hover:bg-primary/90 transition shadow">
            ...
            <div className="relative">
                <div ref={tableContainerRef} className="overflow-x-auto border border-border">
                    <table className="border-collapse text-xs whitespace-nowrap">
                        <thead className="sticky top-0 bg-primary/10 text-primary z-10 uppercase">
                            <tr>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-14">NO</th>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-64">Uraian Pekerjaan</th>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-32">Start Date</th>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-32">Finish Date</th>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-20">Duration</th>
                                <th rowSpan={2} className="border-r border-border p-2 min-w-20">Bobot (%)</th>
                                {Object.entries(monthHeaders).map(([month, colSpan]) => (
                                    <th key={month} colSpan={colSpan} className="border-r border-border p-2 font-semibold">{month}</th>
                                ))}
                                <th rowSpan={2} className="p-2 min-w-40 bg-orange-500/10 text-orange-600">Keterangan</th>
                                <th rowSpan={2} className="p-2 min-w-20 border-l border-border">Aksi</th>
                            </tr>
                            <tr>
                                {timeline.map((w, i) => (
                                    <th key={w.week} className="border-r border-border font-normal w-16 p-1 text-center" title={`${w.startDate.toLocaleDateString('id-ID')} - ${w.endDate.toLocaleDateString('id-ID')}`}>
                                        W{i % 5 + 1}
                        ...
                        <tbody>
                            {tasks.map(task => {
                                ...
                                return (
                                <tr key={task.id} className={`${task.isCategory ? 'bg-muted/50 font-bold' : 'hover:bg-muted/30'}`}>
                                    <td className="border-r border-border p-1 text-center">{taskNo}</td>
                                    <td className="border-r border-border p-1">{isEditing ? <EditableCell value={task.name} onChange={v => handleTaskChange(task.id, 'name', v)} /> : <span className="px-1">{task.name}</span>}</td>
                                    <td className="border-r border-border p-1 text-center">{isEditing && !task.isCategory ? <EditableCell type="date" value={task.startDate} onChange={v => handleTaskChange(task.id, 'startDate', v)} /> : <span className="px-1">{formatDisplayDate(task.startDate)}</span>}</td>
                                    <td className="border-r border-border p-1 text-center">{isEditing && !task.isCategory ? <EditableCell type="date" value={task.endDate} onChange={v => handleTaskChange(task.id, 'endDate', v)} /> : <span className="px-1">{formatDisplayDate(task.endDate)}</span>}</td>
                                    <td className="border-r border-border p-1 text-center bg-green-50 dark:bg-green-900/30">{duration > 0 ? duration : ''}</td>
                                    <td className="border-r border-border p-1 text-center bg-yellow-50 dark:bg-yellow-900/30">{isEditing && !task.isCategory ? <EditableCell type="decimal" className="text-center" value={task.weight} onChange={v => handleTaskChange(task.id, 'weight', v)} /> : <span className="px-1">{task.weight > 0 ? formatDecimal(task.weight) : ''}</span>}</td>
                                    {timeline.map((w, weekIdx) => {
                                        ...
                                        return (
                                            <td key={w.week} className={`border-r border-border p-0 h-full ${hasValue ? 'bg-blue-100 dark:bg-blue-900/40' : ''}`}>
                                                {!task.isCategory ? (
                                                    <DecimalInput
                                                        value={manualValue}
                                                        onNumberChange={(num) => handleManualProgressChange(task.id, weekIdx, num)}
                                                        className="w-16 h-full bg-transparent p-1 text-center outline-none focus:ring-1 focus:ring-primary rounded-none"
                                                        placeholder="-"
                                                    />
                                    ...
                                    <td className="p-1 text-center border-l border-border">
                        ...
                        <tfoot className="sticky bottom-0 bg-primary/10 z-10 text-xs">
                            <tr className="font-bold">
                                <td colSpan={5} className="p-2 text-right border-r border-border">TOTAL</td>
                                <td className="p-2 text-center border-r border-border bg-green-200/50 dark:bg-green-800/50">{formatDecimal(totalWeight)}%</td>
                                {timeline.map((_, i) => <td key={i} className="p-2 text-center border-r border-border">{formatDecimal(calculatedPlannedProgress[i] || 0)}</td>)}
                                <td colSpan={2} className="border-l border-border"></td>
                            </tr>
                             <tr className="font-bold bg-primary/20">
                                <td colSpan={6} className="p-2 text-right border-r border-border">Akumulasi Progress Rencana (%)</td>
                                {cumulativePlanProgress.map((p, i) => <td key={i} className="p-2 text-center border-r border-border">{p > 0 ? formatDecimal(p) : '-'}</td>)}
                                <td colSpan={2} className="border-l border-border"></td>
                            </tr>
                             <tr>
                                <td colSpan={6} className="p-2 text-right border-r border-border">Progress Actual (%)</td>
                                {timeline.map((w, i) => <td key={w.week} className="p-1 text-center border-r border-border">
                                    <DecimalInput 
                                        value={actualWeeklyProgress[i]} 
                                        onNumberChange={(num) => handleActualProgressChange(i, num)} 
                                        className="w-16 bg-yellow-50 dark:bg-yellow-900/30 p-1 text-center outline-none focus:ring-1 focus:ring-primary rounded"/>
                                </td>)}
                                <td colSpan={2} className="border-l border-border"></td>
                            </tr>
                             <tr className="font-bold bg-primary/20">
                                <td colSpan={6} className="p-2 text-right border-r border-border">Akumulasi Progress Actual (%)</td>
                                {cumulativeActualProgress.map((p, i) => <td key={i} className="p-2 text-center border-r border-border">{p > 0 ? formatDecimal(p) : '-'}</td>)}
                                <td colSpan={2} className="border-l border-border"></td>
                            </tr>
                             <tr>
                                <td colSpan={6} className="p-2 text-right border-r border-border font-semibold">Deviasi (%)</td>
                                {timeline.map((w, i) => {
                                    const deviasi = (cumulativeActualProgress[i] || 0) - (cumulativePlanProgress[i] || 0);
                                    return <td key={w.week} className={`p-2 text-center border-r border-border font-bold ${deviasi > 0.01 ? 'text-green-600' : deviasi < -0.01 ? 'text-red-500' : ''}`}>{formatDecimal(deviasi)}</td>
                                })}
                                <td colSpan={2} className="border-l border-border"></td>
                            </tr>
                        </tfoot>
                    ...
                    <div style={{ width: `${numWeeks * 64}px`, height: '100%' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData} margin={{ top: 10, right: 32, left: -32, bottom: 10 }}>
                                ...
                                <Line dataKey="plan" name="Rencana Kumulatif" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={<Dot r={3} fill="hsl(var(--primary))"/>} activeDot={{r: 5}}/>
             ...
             <div className="flex justify-center items-center gap-8 mt-4 text-xs font-semibold">
                <div className="flex items-center gap-2">
                    <div className="w-8 h-1" style={{ backgroundColor: 'hsl(var(--primary))' }}></div>
                    <span className="text-muted-foreground">Rencana Kumulatif</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-8 h-1" style={{ backgroundColor: '#4CAF50' }}></div>
                    <span className="text-muted-foreground">Aktual Kumulatif</span>
                </div>
            </div>
        </div>
    );
